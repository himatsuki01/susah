    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? `${namabot} tidak aktif` : banned ? 'kamu dibanned' : `Himatsuki di sini, Btw ngapain manggil kangen ya bg?`,
                footer,
...
╠═〘 ${namabot} 〙 ═`.trim(), footer, 'Dana', '#viadana', 'GoPay', '#viagopay', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`𝚆𝙰𝚃𝙰𝚂𝙷𝙸 𝙺𝙰𝙶𝚄𝙼𝙸-𝙷𝙸𝙼𝙰𝚃𝚂𝚄𝙺𝙸 || ⏲️ 𝚁𝚄𝙽 𝚃𝙸𝙼𝙴 ${uptime} || 🛰️ 𝙼𝙾𝙳𝙴: ${global.opts['self'] ? '𝙿𝚁𝙸𝚅𝙰𝚃𝙴' : setting.groupOnly ? 'Hanya Grup' : '𝙿𝚄𝙱𝙻𝙸𝙲'}`).catch(_ => _)
